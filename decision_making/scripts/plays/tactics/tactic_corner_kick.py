...
class TacticCornerKick(Sequence):
    def __init__(self, name, my_role, ref_cornerkick_state):
        ...
        ### force to kick their goal
        def delayed_condition_generator(target_role, delay = 3):
            _target_role = target_role
            _endtime = time.time() + delay
            def delayed_condition(): # <- a closure
                if _endtime > time.time():
                    # delayed
                    return False
                return True
            return delayed_condition

        target_role = "CONST_THEIR_GOAL"

        seq = ConditionalSequence(name='pass_to_their_goal', condition=delayed_condition_generator(target_role, 7)) #reset_after=True
        # coord = Coordinate()
        # coord.set_approach_to_shoot(my_role, target='CONST_THEIR_GOAL')
        coord1 = Coordinate()
        coord1.set_interpose(base='Ball', target=target_role, to_dist = -0.3)
