...
from pi_trees_lib_extensions import ConditionalSequence, OverwriteSequenceCondition, Print, ModeSelector, ToggleInplayMode, ResetState, TriggerKicked
...
class TacticInplayReceiveAndShoot(Sequence):
    def __init__(self, name, my_role, ref_mode, ref_kickstate, shoot_if_mode_is_true, *args, **kwargs):
        ...
        coord_receive.set_receive_ball(my_role, 30.0, force_receive=True, target='CONST_THEIR_GOAL')
        p = ParallelOne('drive_to_receive')
        p.add_child(Print('%s is receiving'%(my_role)))
        p.add_child(DynamicDrive('drive_to_receive', my_role, coord_receive))
        receive_seq.add_child(p)
