...
def publish():
    ...
    if keep_base_pose_publisher is not None:
        if WorldModel._keep_base_pose is not None:
            pose_stamped = PoseStamped()
            pose_stamped.pose.position.x = WorldModel._keep_base_pose.x
            pose_stamped.pose.position.y = WorldModel._keep_base_pose.y
            pose_stamped.header.stamp = rospy.Time.now()
            keep_base_pose_publisher.publish(pose_stamped)  
...
if __name__ == '__main__':
    ...
    if b_debug_publishing:
        ...
        keep_base_pose_publisher = rospy.Publisher('keep_base_pose', PoseStamped, queue_size=10)
